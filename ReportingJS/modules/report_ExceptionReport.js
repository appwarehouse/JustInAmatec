module.exports = function (site, start, end, bigquery, exceptionType) {
    try {

        async function query() {
            const query = "SELECT 'Missing Entry' as Exception_Type, CAST((Entry_Time) AS STRING) ,CAST((Exit_Time) AS STRING), driver_card_photo_url,exit_driver_card_photo_url,exit_driver_name,exit_driver_id_no,entry_time,exit_time,device_name, exit_device_name,vehicle_licence_no as VehicleDetails, exit_vehicle_licence_no as exit_VehicleDetails,	driver_name as DriverDetails, exit_device_name as exit_DriverDetails, site_name " +
